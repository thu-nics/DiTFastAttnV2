...
from ditfastattn_api.modules.ilp_residual import solve_ip
...
class MMDiTFastAttnProcessor:
    ...
    def calib_qkv_process_func(self, query, key, value, forward_args):
        ...
        for candidate in candidates:
            if "arrow_attn" in candidate:
                ...
                if "residual" in candidate:
                    hidden_states = hidden_states + self.cached_residual.to(query.device)
        ...
        if "cfg_share" not in candidates:
            ...
            update_residual_mask = torch.tensor([False] * 24)
            ...
            for head_method in head_method_list:
                ...
                if ws == "0":
                    ...
                    if int(ws) < 0:
                        update_residual_mask[head] = True
            ...
            self.residual_mask[self.stepi-1] = update_residual_mask & self.residual_mask[self.stepi-1]
            output[:,:,update_residual_mask,:] = output[:,:,update_residual_mask,:] + self.cached_residual[:,:,update_residual_mask,:]
            full_attention_idx = wt == ((S - 333) * 2)
            if not sum(full_attention_idx)==0:
                temp_window_output = self.window_func(query[:,full_attention_idx,:,:],
                                                    key[:,full_attention_idx,:,:],
                                                    value[:,full_attention_idx,:,:],
                                                    self.block_mask[8]
                                                    ).transpose(1, 2)
                self.cached_residual[:,:,full_attention_idx,:] = output[:,:,full_attention_idx,:] - temp_window_output
            # reset other residual
            self.cached_residual[:,:,~full_attention_idx,:] = float('-inf')
            self.residual_mask[self.stepi] = full_attention_idx
            # breakpoint()
        else:
    ...
    def raw_qkv_process_func(self, query, key, value, forward_args):
        ...
        if self.stepi == 0:
            ...
            if window_size_factor not in self.block_mask.keys():
                ...
                block_mask = create_block_mask(sliding_window_mask, None, None, S, S, device="cuda", _compile=True, BLOCK_SIZE=128)
                self.block_mask[window_size_factor] = block_mask
            window_hidden_states = self.window_func(
                query, key, value, block_mask=self.block_mask[window_size_factor]
            ).transpose(1, 2)
            torch.cuda.synchronize()
            self.cached_residual = hidden_states - window_hidden_states
            self.residual_mask[self.stepi] = torch.tensor([True] * 24)
        return hidden_states
    ...
    def qkv_process_perhead_func(self, query, key, value, forward_args):
        ...
        torch.cuda.synchronize()
        if self.stepi != 0 and sum(self.residual_mask[self.stepi - 1]) != 0:
            if output[:,:,self.residual_mask[self.stepi - 1],:].shape != self.cached_residual.shape:
                breakpoint()
            output[:,:,self.residual_mask[self.stepi - 1],:] = output[:,:,self.residual_mask[self.stepi - 1],:] + self.cached_residual
        ...
        if self.stepi in self.residual_mask.keys():
            if sum(self.residual_mask[self.stepi]) != 0:
                temp_window_output = self.window_func(query[:,self.residual_mask[self.stepi],:,:],
                                                    key[:,self.residual_mask[self.stepi],:,:],
                                                    value[:,self.residual_mask[self.stepi],:,:],
                                                    self.block_mask[8]
                                                    ).transpose(1, 2)
                self.cached_residual = output[:,:,self.residual_mask[self.stepi],:] - temp_window_output
    ...
    def raw_qkv_process_after_calib_func(self, query, key, value, forward_args):
        ...
        if self.stepi in self.residual_mask.keys():
            if sum(self.residual_mask[self.stepi]) != 0:
                ...
                self.cached_residual = hidden_states[:,:,self.residual_mask[self.stepi],:] - temp_window_output
        if self.stepi+1 in self.output_share_dict.keys():
