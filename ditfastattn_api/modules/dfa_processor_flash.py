...
def headwise_cfg_attention(query, key, value, head_mask, window_sizes):
    ...
    if query_replace.size(2) > 0:  # 如果有需要替换的 head
        ...
        attention_first_half = dfav2.headwise_window_attn(query_first_half.transpose(1,2), 
    ...
    if query_normal.size(2) > 0:  # 如果有不需要替换的 head
        attention_normal = dfav2.headwise_window_attn(query_normal.transpose(1,2), 
    ...
    def calib_qkv_process_func(self, query, key, value, forward_args):
        ...
        if "cfg_share" not in candidates:
            ...
            if wt.any() == 0:
                ...
                output = dfav2.headwise_window_attn(
                    query.transpose(1, 2), 
                    key.transpose(1, 2), 
                    value.transpose(1, 2), 
                    window_sizes=wt
