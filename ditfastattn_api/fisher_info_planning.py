...
def set_wan_compression_plan(pipe, dataloader, dfa_config, model_misc, alpha):
    for i, kwargs in enumerate(dataloader()):
        print(f">>> calibration sample {i} <<<")
        model_misc.inference_fn_plan_update_iop_per_head_wan(pipe, dfa_config, alpha, **kwargs)
